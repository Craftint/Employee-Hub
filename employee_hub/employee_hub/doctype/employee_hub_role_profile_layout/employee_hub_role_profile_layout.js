...
// Varied row counts on purpose (3 to 6), with a spread of different
...
class RoleProfileLayoutPreview {
    constructor(frm) {
        this.frm = frm;
        this.activeTab = null;
        this.pendingLayout = this.load_from_doc();
        this.customizeMode = false; // per spec: preview opens in plain view first, not customize
    ...
    load_from_doc() {
        return (this.frm.doc.layout || []).map((row) => ({
            scope: row.scope,
            tab: row.tab,
    ...
    sync_to_doc() {
        ...
        // single interaction — the same fix already applied to
        // save_employee_hub_layout server-side, ported here since this
        // one runs entirely client-side. Wiping-and-rebuilding made every
        // hide/reorder/move/duplicate look like "33 rows removed, 33 rows
        // added" in the version history, regardless of how small the
        // actual change was.
        const existingByKey = {};
        (this.frm.doc.layout || []).forEach((row) => {
        ...
        this.frm.doc.layout = resultRows;

        this.frm.refresh_field('layout');
    ...
    ordered_visible_tabs() {
        ...
        return RP_HUB_TABS.filter((t) => !map[t.key] || !map[t.key].is_hidden).sort((a, b) => {
    ...
    render_tab() {
        ...
        let statItems = this.pendingLayout.filter((i) => i.scope === 'Card' && i.tab === this.activeTab && RP_CARD_REGISTRY[i.card_key] && RP_CARD_REGISTRY[i.card_key].category === 'stat');
        let listItems = this.pendingLayout.filter((i) => i.scope === 'Card' && i.tab === this.activeTab && RP_CARD_REGISTRY[i.card_key] && RP_CARD_REGISTRY[i.card_key].category !== 'stat');
        if (!this.customizeMode) {
    ...
    render_card($row, item) {
        const meta = RP_CARD_REGISTRY[item.card_key];
        if (!meta) return;
        ...
        if (meta.kind === 'stat') {
            ...
            const rows = RP_SAMPLE_BIRTHDAYS.map(
            ...
            const sampleRows = RP_SAMPLE_LIST_ROWS[item.card_key] || [['Sample row 1', '', ''], ['Sample row 2', '', '']];
    ...
    open_tab_picker($card, action) {
        ...
        const otherTabs = this.ordered_visible_tabs()
            .concat(RP_HUB_TABS.filter((t) => !this.ordered_visible_tabs().some((v) => v.key === t.key)))
    ...
    duplicate_card(cardKey, targetTab) {
        ...
        this.sync_to_doc();
        frappe.show_alert({ message: __('Also added to {0}', [(RP_HUB_TABS.find((t) => t.key === targetTab) || {}).label || targetTab]), indicator: 'blue' });
    }
...
// Tries several known selectors for the form's own left sidebar (Assigned
// To / Attachments / Tags / Share, etc — different from the workspace
// navigation sidebar) across Frappe versions, hiding whichever is found.
...
// the breadcrumb), not a separate control of ours. Uses a plain class
// (not an inline style, not !important) specifically so it doesn't fight
// or override whatever mechanism that native toggle already uses.
function rp_collapse_form_sidebar(collapse) {
    const selectors = [
        '.layout-side-section',
        '.form-sidebar',
...
const RP_FETCH_ICON = `<svg width="13" height="13" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
...
const RP_RESET_ICON = `<svg width="13" height="13" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
...
function rp_update_header_state(frm) {
    if (!frm._rp_header || !document.body.contains(frm._rp_header[0])) {
        clearInterval(frm._rp_state_interval); // form navigated away from — stop checking
        return;
    }

    const hasRows = (frm.doc.layout || []).length > 0;

    const $resetBtn = frm._rp_header.find('.hub-rp-reset-btn');
    $resetBtn.find('.hub-rp-reset-icon').html(hasRows ? RP_RESET_ICON : RP_FETCH_ICON);
    ...
    // least one row — an empty table has no tabs to show at all, and
    // "Customize" specifically was asked to only appear once real data
    // exists, checked live rather than tied to whether the form has been
    // saved.
    const $toggleBtn = frm._rp_header.find('.hub-rp-toggle-btn');
    const $customizeBtn = frm._rp_header.find('.hub-rp-customize-btn');
    $toggleBtn.attr('data-empty', !hasRows ? '1' : '').toggleClass('hub-rp-btn-disabled', !hasRows);
    if (!hasRows && frm._rp_in_preview) {
        // Data was removed while previewing — drop back to the table view.
        frm._rp_in_preview = false;
        frm._rp_preview_wrapper.hide();
        frm.fields_dict.layout.$wrapper.show();
        $customizeBtn.hide();
        rp_collapse_form_sidebar(false);
...
frappe.ui.form.on("Employee Hub Role Profile Layout", {
    async refresh(frm) {
        ...
        if (!enabled) {
            ...
            return; // the header buttons below would only fail validation on save anyway
        }

        if (!frm.fields_dict.layout || !frm.fields_dict.layout.$wrapper) return;
        const $gridWrapper = frm.fields_dict.layout.$wrapper;

        if (!frm._rp_header) {
            // Built once — a small header strip directly above the Layout
            // Items table (not in the page toolbar), per spec.
            frm._rp_header = $(`
            ...
            $gridWrapper.before(frm._rp_header);

            frm._rp_preview_wrapper = $('<div class="hub-rp-preview-wrapper" style="display:none;"></div>');
            $gridWrapper.after(frm._rp_preview_wrapper);

            const $toggleBtn = frm._rp_header.find('.hub-rp-toggle-btn');
            const $customizeBtn = frm._rp_header.find('.hub-rp-customize-btn');

            const setToggleLabel = () => {
                $toggleBtn.find('span').first().text(frm._rp_in_preview ? __("Table View") : __("Preview"));
                $toggleBtn.toggleClass('hub-rp-btn-active', !!frm._rp_in_preview);
            };
            const setCustomizeLabel = () => {
                $customizeBtn.toggleClass('hub-rp-btn-active', !!(frm.rp_preview && frm.rp_preview.customizeMode));
            };

            $toggleBtn.on('click', () => {
                if ($toggleBtn.attr('data-empty')) {
                    frappe.msgprint(__('There\'s nothing to preview yet — use Fetch Default Layout, or add at least one row to the table below first.'));
                    return;
                }
                frm._rp_in_preview = !frm._rp_in_preview;
                if (frm._rp_in_preview) {
                    $gridWrapper.hide();
                    frm._rp_preview_wrapper.show();
                    frm.rp_preview.customizeMode = false; // always opens in plain view, not customize
                    frm.rp_preview.render_root(frm._rp_preview_wrapper);
                    $customizeBtn.show();
                    rp_collapse_form_sidebar(true); // wider preview by default; bring it back with Frappe's own ☰ sidebar toggle
                } else {
                    frm._rp_preview_wrapper.hide();
                    $gridWrapper.show();
                    $customizeBtn.hide();
            ...
            $customizeBtn.on('click', () => {
                if (!frm.rp_preview) return;
                frm.rp_preview.customizeMode = !frm.rp_preview.customizeMode;
                frm.rp_preview.render_root(frm._rp_preview_wrapper);
                setCustomizeLabel();
            });

            frm._rp_header.find('.hub-rp-reset-btn').on('click', () => {
                const isEmpty = !(frm.doc.layout || []).length;
                frappe.confirm(
                    isEmpty
                        ? __("Fetch the current Global Default Layout into this record?")
                    ...
                    () => {
                        frappe.call("employee_hub.employee_hub.api.get_global_default_layout_items").then((r) => {
                            ...
                            frm.save().then(() => {
                                frappe.show_alert({ message: __(isEmpty ? "Fetched the default layout" : "Reset to the default layout"), indicator: "green" });
                                if (frm.rp_preview) {
                                    frm.rp_preview.pendingLayout = frm.rp_preview.load_from_doc();
                                    if (frm._rp_in_preview) frm.rp_preview.render_root(frm._rp_preview_wrapper);
                                }
                                rp_update_header_state(frm);
                            });
                        });
                    }
                );
            ...
            // Re-checked continuously (not just after Save) so Preview/
            // Customize become available and the Fetch/Reset button's
            // label switches the instant a row exists — whether it got
            // there by fetching, or by the user adding one directly into
            // the raw grid themselves.
            frm._rp_state_interval = setInterval(() => rp_update_header_state(frm), 700);
            rp_update_header_state(frm);
        ...
        if (!frm.rp_preview) {
            frm.rp_preview = new RoleProfileLayoutPreview(frm);
        } else {
            frm.rp_preview.pendingLayout = frm.rp_preview.load_from_doc();
        }
    },
